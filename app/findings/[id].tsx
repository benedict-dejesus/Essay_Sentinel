...
export default function FindingsScreen() {
  ...
  const statusDetail = review.revisionGuidanceSuggested ? "The configured marker or mechanics threshold was reached. Use the suggestions below as revision prompts, not a failure grade." : "The current rules did not reach a revision threshold. This is not an authorship verification or grade.";
  ...
  return <ScreenContainer edges={["top", "bottom", "left", "right"]}>
    <FlatList
      ...
      ListHeaderComponent={<View>
        ...
        <View className="mt-7 rounded-2xl border border-border bg-surface p-4"><Text className="text-sm font-semibold text-foreground">Use this as a starting point</Text><Text className="mt-1 text-sm leading-5 text-muted">These are fixed language-pattern and mechanics observations, not an AI detector and not proof of authorship. Review assignment context and speak with the student before making any decision.</Text></View>
        <Text className="mt-7 mb-3 text-lg font-bold text-foreground">Matched marker categories</Text>
      </View>}
      ListEmptyComponent={<View className="rounded-3xl border border-dashed border-border bg-surface p-5"><Text className="text-base font-semibold text-foreground">No configured markers matched</Text><Text className="mt-2 text-sm leading-5 text-muted">This result does not verify authorship; it only means the current rules did not find a configured pattern.</Text></View>}
