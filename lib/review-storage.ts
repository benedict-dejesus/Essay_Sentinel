...
function hydrateReview(review: SavedReview): SavedReview {
  if (review.statistics && review.recommendations && typeof review.revisionGuidanceSuggested === "boolean" && review.ruleSetId && review.ruleSetName) return review;
  return { ...review, ...analyzeEssay(review.text) };
}
...
export async function saveReview(review: Omit<SavedReview, "id" | "createdAt">): Promise<SavedReview> {
  const createdAt = new Date().toISOString();
  const saved: SavedReview = { ...review, id: `${Date.now()}-${Math.random().toString(16).slice(2)}`, createdAt };
